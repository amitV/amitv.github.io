...
const Connect4 = () => {
  ...
  const renderCell = (row, col) => {
    ...
    let cellPieceClassName = "cell-piece cell-empty";
    if (cellContent === PLAYER_1 || (isCurrentlyDropping && currentPlayer === PLAYER_1)) {
      cellPieceClassName = "cell-piece cell-red";
    } else if (cellContent === PLAYER_2 || (isCurrentlyDropping && currentPlayer === PLAYER_2)) {
      cellPieceClassName = "cell-piece cell-yellow";
    }
    ...
    return (
      <div key={`${row}-${col}`} className="game-cell">
        ...
        <div className={cellPieceClassName}></div>
      </div>
    );
  };
